...
### 1.1 Environment-Related Issues

- **Issue 1**: Running `roslaunch` results in the error `package 'p1_pro' not found`
...
### 1.2 Network Communication Issues

- **Issue 1**: Error: "Trying to connect to real ultraArm P1... Timeout"
...
- **Issue 1**: **Runtime Error** ModuleNotFoundError: No module named 'ultraArm_P1_lib'
  ...
  - **Issue 2**: **Runtime Error** ImportError: cannot import name 'P1Client' from 'ultraArm_P1_lib'
...
- **Problem 2**: After opening rviz, the model is incomplete, and the terminal continuously outputs a warning message similar to: `Warning [Invalid frame ID "base" passed to canTransform argument target frame frame does not exist rviz] rviz at line 133 in /tmp/binarydeb/ros-foxy-tf2-0.13.13/src/buffer core.cpp`
  ...
  - Check whether the robot can read angles and coordinates normally.
    ...
    mc = P1Client('192.168.0.232', 4500)
...
- **Issue 1**: Running `ros2 launch` results in the error `package 'p1_pro' not found`
  - **Cause**: The ROS workspace was not compiled correctly or the environment variables were not loaded
  - **Solution**:
    ```bash
    cd ~/colcon_ws
