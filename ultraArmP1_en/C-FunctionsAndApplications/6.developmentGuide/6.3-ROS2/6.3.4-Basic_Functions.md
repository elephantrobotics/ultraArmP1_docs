...
Refer to [Installing ultraArm P1 p1_ros2](./6.3.1-Environment_Setup.md)
...
<!-- >> **Note: The ultraArm_P1_lib driver library version must be greater than 4.0.0. Moveit2 only supports the ROS2 Humble version. Please use the code in the corresponding branch to run.** -->
...
```bash
ros2 launch p1_arm_moveit2 demo.launch.py
```
...
poster="" data-setup='{"aspectRatio":"16:9"}'>
  ...
  <source src="../../../../resources/C-FunctionsAndApplications/6-SoftwareDevelopment/6.3-ROS2p1_arm-moviet2.mp4"></video>

如果想让真正的机械臂同步执行计划，则需要打开另一个命令行并运行：

```bash
# 默认串口名为"/dev/ttyUSB0"，波特率为115200.
ros2 run p1_arm_moveit2_control sync_plan
```
...
def listener_callback(self, msg):
    ...
    self.p1_arm.send_angles(data_list, 25)
