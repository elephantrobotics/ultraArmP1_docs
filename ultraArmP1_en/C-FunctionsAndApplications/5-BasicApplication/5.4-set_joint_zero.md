...
- **Software and Environment**
  ...
  - The `ultraArm_P1_lib` library installed (using the `pip install ultraArm_P1_lib` terminal command)
  - Ensure that the ultraArm P1 is properly powered on and in standby mode.
  - **Note**: The P1 server automatically starts upon powering on; no manual operation is required.

- **Network Configuration**
  - ultraArm P1 default IP address: `192.168.0.232`
...
This example is used to perform **joint zero-point calibration** on the ultraArm P1 robotic arm.
...
```python
import time
from ultraArm_P1_lib import P1Client
...
def read_angles(p1_arm, retry=3, delay=0.2):
    """Safely read joint angles"""
    for _ in range(retry):
        angles = p1_arm.get_angles()
...
def main():
    p1_arm = P1Client('192.168.0.232', 4500)

    # Power-on check
    if p1_arm.is_power_on() != 1:
    ...
    if safe_input("The robotic arm is about to relax all joints. Please hold on tight! Type "yes" and press Enter to select "Relax All Joints Now"."):
        p1_arm.set_motor_enabled(254, 0)
        time.sleep(0.02)
    else:
    ...
    if safe_input("Please manually drag each joint to the zero mark, type "yes" and press Enter to "Lock the joints and perform zero-point calibration"."):
        p1_arm.set_motor_enabled(254, 1)
        time.sleep(0.02)
        # Perform joint zero-position calibration
        for i in range(1, 7):
            p1_arm.set_servo_calibration(i)
            time.sleep(0.05)

        # Calibration complete, reading angle
        angles = read_angles(p1_arm)
